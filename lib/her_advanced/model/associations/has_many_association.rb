    ...
    module Associations
      class HasManyAssociation < Association
        ...
        def self.attach(klass, name, opts)
          ...
          klass.class_eval <<-RUBY, __FILE__, __LINE__ + 1
            def #{name}
              ...
              cached_data || instance_variable_set(cached_name, HerAdvanced::Model::Associations::HasManyAssociation.proxy(self, #{opts.inspect}))
            end
          RUBY
        end
        ...
        def self.parse(association, klass, data)
          ...
          { association[:name] => HerAdvanced::Model::Attributes.initialize_collection(klass, :data => data[data_key]) }
        end
        ...
        def create(attributes = {})
          ...
          if resource.save
            @parent.attributes[@name] ||= HerAdvanced::Collection.new
        ...
        def assign_nested_attributes(attributes)
          ...
          @parent.attributes[@name] = HerAdvanced::Model::Attributes.initialize_collection(@klass, :data => data)
        end
      end
    end
  end
