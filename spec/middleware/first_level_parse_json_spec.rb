...
describe HerAdvanced::Middleware::FirstLevelParseJSON do
  ...
  it 'ensures that malformed JSON throws an exception' do
    expect { subject.parse(body_with_malformed_json) }.to raise_error(HerAdvanced::Errors::ParseError, 'Response from the API must behave like a Hash or an Array (last JSON response was "wut.")')
  end

  it 'ensures that invalid JSON throws an exception' do
    expect { subject.parse(body_with_invalid_json) }.to raise_error(HerAdvanced::Errors::ParseError, 'Response from the API must behave like a Hash or an Array (last JSON response was "true")')
  end
